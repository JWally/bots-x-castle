...
| **`npm run decrypt`** | **Cipher reversal demo.** Captures one live Castle token from x.com via a btoa hook stack-filtered to `at tt`, then decrypts using the bundle's own inverse function (`tv()`) we extracted into `castle-cipher.mjs`. Confirms `tt() ↔ tv()` is a clean inverse pair — Castle ships its own decryptor in the same bundle for build-time string deobfuscation, and it inverts the runtime token byte-for-byte. Renders the TLV-shaped plaintext. | When you want to see what's actually inside a Castle token. |
| `npm run tt-trace` | In-flight bundle patch wrapping every `function tt(n){…}` declaration to log per-call input char codes + output base64. Reveals individual per-field plaintexts (timestamps, hex hashes, empty literals, the aggregated TLV payload). | When you want the per-field plaintexts directly, without going through the token. |
